...
export function AdminDashboardApp() {
  ...
  if (checkingAuth) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-slate-200">
        ...
        <p className="mt-4 text-sm text-slate-400">Authenticating administrator...</p>
      </div>
    );
  }
  ...
  if (!isAdmin) {
    return (
      <main className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden select-none">
        {/* Glow ambient background */}
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] rounded-full bg-blue-900/20 blur-[100px] pointer-events-none" />
        <div className="absolute bottom-1/4 left-1/3 w-[250px] h-[250px] rounded-full bg-indigo-900/20 blur-[80px] pointer-events-none" />

        <div className="w-full max-w-md bg-slate-900/60 border border-slate-800 backdrop-blur-md rounded-2xl p-8 shadow-[0_8px_30px_rgba(0,0,0,0.5)] relative z-10">
          <div className="text-center mb-8">
            <div className="mx-auto w-12 h-12 bg-blue-500/10 border border-blue-500/20 rounded-xl flex items-center justify-center mb-3">
              <Shield className="h-6 w-6 text-blue-400" />
            </div>
            <h1 className="text-slate-100 text-2xl font-black tracking-tight uppercase">
              Admin Portal
            </h1>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full h-[46px] border border-slate-800 bg-slate-950/80 text-slate-100 rounded-lg pl-10 pr-4 text-sm placeholder-slate-600 focus:border-blue-500 outline-none transition-all"
                />
                <Key className="absolute left-3.5 top-[15px] h-4 w-4 text-slate-500" />
              </div>
            </div>
      ...
      <header className="border-b border-slate-900 bg-slate-900/30 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 bg-blue-500/10 border border-blue-500/30 rounded-lg flex items-center justify-center">
              <Shield className="h-5 w-5 text-blue-400" />
            </div>
            <div>
              <span className="font-bold text-lg tracking-tight bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">TicketHub</span>
              <span className="text-[10px] bg-blue-500/10 text-blue-400 border border-blue-500/20 px-1.5 py-0.5 rounded font-extrabold tracking-wider uppercase ml-2.5">ADMIN</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 border border-slate-800 bg-slate-900/60 rounded-lg text-slate-400 hover:text-slate-200 transition-colors"
              title="Refresh Data"
            >
            ...
            <button
              ...
              className="h-9 px-4 border border-slate-850 bg-slate-900/60 hover:bg-slate-900 text-slate-300 hover:text-slate-100 rounded-lg text-xs font-semibold transition-colors flex items-center gap-2 cursor-pointer"
            >
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-slate-900/40 border border-slate-900 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Total Accesses</span>
              <Users className="h-4 w-4 text-blue-400" />
            </div>
            <p className="text-3xl font-black text-slate-100">{stats.total}</p>
            <p className="text-[10px] text-slate-500 mt-1">Generated login keys</p>
          </div>
          <div className="bg-slate-900/40 border border-slate-900 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-3xl font-black text-slate-100">{stats.active}</p>
            <p className="text-[10px] text-slate-500 mt-1">Currently signed-in users</p>
          </div>
          <div className="bg-slate-900/40 border border-slate-900 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Pending Keys</span>
              <Key className="h-4 w-4 text-amber-400" />
            </div>
            <p className="text-3xl font-black text-slate-100">{stats.pending}</p>
            <p className="text-[10px] text-slate-500 mt-1">Unused code authorizations</p>
          </div>
          <div className="bg-slate-900/40 border border-slate-900 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Revoked/Expired</span>
              <Clock className="h-4 w-4 text-rose-400" />
            </div>
            <p className="text-3xl font-black text-slate-100">{stats.terminated + stats.expired}</p>
            <p className="text-[10px] text-slate-500 mt-1">{stats.terminated} terminated, {stats.expired} expired</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Tab Selector */}
            <div className="bg-slate-900/20 border border-slate-900 p-1 rounded-xl flex gap-1">
              <button
            ...
            {/* User Accounts Tab */}
            {activeTab === 'users' && (
              <div className="bg-slate-900/40 border border-slate-900 rounded-2xl overflow-hidden shadow-md">
                {/* Table Header Controls */}
                <div className="p-5 border-b border-slate-900 flex flex-wrap gap-4 items-center justify-between">
                  ...
                  <div className="relative w-64">
                    <input
                      ...
                      className="w-full h-9 border border-slate-800 bg-slate-950/80 text-slate-100 rounded-lg pl-9 pr-4 text-xs focus:border-blue-500 outline-none transition-all"
                    />
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse text-left">
                    <thead>
                      <tr className="border-b border-slate-900/80 bg-slate-900/10 text-slate-400 text-[10px] font-extrabold uppercase tracking-wider">
                          ...
                          <tr key={u.id} className="hover:bg-slate-900/20 transition-colors">
                            ...
                            <td className="py-3.5 px-5 font-mono">
                              <div className="flex items-center gap-1.5">
                                <span className="bg-slate-950 px-2 py-0.5 rounded border border-slate-850 font-bold text-blue-400">
                                  {u.password}
                                </span>
                                <button
                            ...
                            <td className="py-3.5 px-5 font-semibold text-slate-300">
                              {u.loginMode === 'multiple' ? (
                                <span className="text-emerald-400">Multiple</span>
                              ) : (
                                <span className="text-blue-400">Single</span>
                              )}
                            </td>
                            <td className="py-3.5 px-5 text-slate-400">
                              {u.activatedAt ? (
                                <div>
                                  ...
                                  <p className="text-[10px] text-slate-500">Expires {new Date(u.expiresAt).toLocaleDateString()}</p>
                                </div>
                              ) : (
                                <span className="text-slate-600">Pending Login</span>
                              )}
                            </td>
                            <td className="py-3.5 px-5 text-slate-400 max-w-[120px] truncate">
                              {u.deviceInfo ? (
                              ...
                              <div className="flex items-center justify-end gap-2">
                                {u.status === 'active' && (
                                  <button
                                    ...
                                    className="px-2.5 py-1 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 hover:border-rose-500/30 rounded-md font-semibold text-[10px] uppercase tracking-wider cursor-pointer transition-colors"
                                  >
                                    Terminate
                                  </button>
                                )}
                                <button
                                  ...
                                  className="p-1 text-slate-500 hover:text-rose-400 border border-transparent hover:border-slate-800 rounded-md transition-colors cursor-pointer"
                                  title="Delete Record"
                                >
          ...
          <div className="space-y-6">
            <div className="bg-slate-900/40 border border-slate-900 rounded-2xl p-6 shadow-md">
              <h3 className="font-bold text-sm uppercase tracking-wider text-slate-200 mb-5 flex items-center gap-2">
              ...
              <form onSubmit={handleCreateUser} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full h-[40px] border border-slate-800 bg-slate-950/80 text-slate-100 rounded-lg px-3.5 text-xs placeholder-slate-600 focus:border-blue-500 outline-none transition-all"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full h-[40px] border border-slate-800 bg-slate-950/80 text-slate-100 rounded-lg px-3 text-xs focus:border-blue-500 outline-none transition-all"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full h-[40px] border border-slate-800 bg-slate-950/80 text-slate-100 rounded-lg px-3.5 text-xs placeholder-slate-650 focus:border-blue-500 outline-none transition-all font-mono"
                  />
                </div>
              ...
              {lastCreatedUser && (
                <div className="mt-6 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl relative overflow-hidden">
                  <div className="absolute top-0 right-0 h-10 w-10 bg-emerald-500/5 rounded-full blur-md" />
                  ...
                  <div className="space-y-1.5 text-xs text-slate-200">
                    <p>
                      <strong className="text-slate-400">Email:</strong> {lastCreatedUser.email}
                    </p>
                    <p>
                      <strong className="text-slate-400">Passcode:</strong>{' '}
                      <span className="font-mono bg-slate-950 text-emerald-400 px-1.5 py-0.5 rounded border border-slate-850 font-bold">
                  ...
                  <button
                    ...
                    className="w-full mt-3.5 h-[34px] border border-emerald-500/25 hover:border-emerald-500/40 bg-emerald-500/5 hover:bg-emerald-500/10 text-emerald-400 hover:text-emerald-300 font-semibold text-[11px] rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    {copiedId === 'last_gen' ? (
                      <>
            ...
            <div className="bg-slate-900/40 border border-slate-900 rounded-2xl p-6 shadow-md space-y-4">
              <h3 className="font-bold text-xs uppercase tracking-wider text-slate-400">
                How One-Time Access works
              </h3>
              <div className="space-y-3.5 text-[11px] text-slate-400 leading-relaxed">
                <div className="flex gap-2">
                  <span className="h-5 w-5 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center shrink-0 font-bold">1</span>
                  <p>When you generate an authorization, it is marked as <strong className="text-slate-355">Pending Login</strong>. The timer has not started yet.</p>
                </div>
                <div className="flex gap-2">
                  <span className="h-5 w-5 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center shrink-0 font-bold">2</span>
                  <p>The user logs in at the homepage. The status switches to <strong className="text-slate-355">Active Session</strong> and starts their time duration limit.</p>
                </div>
                <div className="flex gap-2">
                  <span className="h-5 w-5 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center shrink-0 font-bold">3</span>
                  <p>A user cannot sign in a second time using that same passcode, preventing credential sharing or multi-device reuse.</p>
                </div>
                <div className="flex gap-2">
                  <span className="h-5 w-5 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center shrink-0 font-bold">4</span>
                  <p>To end an active session immediately, click <strong className="text-rose-455">Terminate</strong>. They will be logged out on their next action.</p>
                </div>
              </div>
            </div>
          </div>
